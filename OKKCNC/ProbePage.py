...
class CameraGroup(CNCRibbon.ButtonGroup):
    ...
    def switchCommand(self, event=None):
        wx = OCV.CD["wx"]
        wy = OCV.CD["wy"]
        dx = OCV.TK_CANVAS_F.canvas.cameraDx
        dy = OCV.TK_CANVAS_F.canvas.cameraDy
        z  = OCV.TK_CANVAS_F.canvas.cameraZ
        if self.switch.get():
            self.switchButton.config(image=OCV.icons["endmill32"])
            self.sendGCode("G92X%gY%g"%(dx+wx,dy+wy))
            OCV.TK_CANVAS_F.canvas.cameraSwitch = True
        else:
            self.switchButton.config(image=OCV.icons["camera32"])
            self.sendGCode("G92.1")
            OCV.TK_CANVAS_F.canvas.cameraSwitch = False
        if z is None:
    ...
    def edgeDetection(self):
        OCV.TK_CANVAS_F.canvas.cameraEdge = self.edge.get()

    #-----------------------------------------------------------------------
    def freezeImage(self):
        OCV.TK_CANVAS_F.canvas.cameraFreeze(self.freeze.get())
...
class CameraFrame(CNCRibbon.PageFrame):
    ...
    def updateValues(self, *args):
        OCV.TK_CANVAS_F.canvas.cameraAnchor = self.cameraAnchor()
        try: OCV.TK_CANVAS_F.canvas.cameraRotation = float(self.rotation.get())
        except ValueError: pass
        try: OCV.TK_CANVAS_F.canvas.cameraXCenter = float(self.xcenter.get())
        except ValueError: pass
        try: OCV.TK_CANVAS_F.canvas.cameraYCenter = float(self.ycenter.get())
        except ValueError: pass
        try: OCV.TK_CANVAS_F.canvas.cameraScale = max(0.0001, float(self.scale.get()))
        except ValueError: pass
        try: OCV.TK_CANVAS_F.canvas.cameraR = float(self.diameter.get())/2.0
        except ValueError: pass
        try: OCV.TK_CANVAS_F.canvas.cameraDx = float(self.dx.get())
        except ValueError: pass
        try: OCV.TK_CANVAS_F.canvas.cameraDy = float(self.dy.get())
        except ValueError: pass
        try:
            OCV.TK_CANVAS_F.canvas.cameraZ  = float(self.z.get())
        except ValueError:
            OCV.TK_CANVAS_F.canvas.cameraZ  = None
        OCV.TK_CANVAS_F.canvas.cameraUpdate()
