...
class Camera:
    ...
    def __init__(self, prefix=""):
        ...
        self.prefix  = prefix
        self.idx     = IniFile.get_int("Camera", prefix)
    ...
    def _getCameraProperties(self, prefix):
        ...
        try:
            POSSIBLE_PROPERTIES = {
                'height'     : (IniFile.get_int, cv.CAP_PROP_FRAME_HEIGHT),
                'width'      : (IniFile.get_int, cv.CAP_PROP_FRAME_WIDTH),
                'fps'        : (IniFile.get_int, cv.CAP_PROP_FPS),
                'codec'      : (IniFile.get_str, cv.CAP_PROP_FOURCC),
                'brightness' : (IniFile.get_int, cv.CAP_PROP_BRIGHTNESS),
                'contrast'   : (IniFile.get_int, cv.CAP_PROP_CONTRAST),
                'saturation' : (IniFile.get_int, cv.CAP_PROP_SATURATION),
                'hue'        : (IniFile.get_int, cv.CAP_PROP_HUE),
                'gain'       : (IniFile.get_int, cv.CAP_PROP_GAIN),
                'exposure'   : (IniFile.get_int, cv.CAP_PROP_EXPOSURE),
            }
        except AttributeError:
            POSSIBLE_PROPERTIES = {
                'height'     : (IniFile.get_int, cv.cv.CV_CAP_PROP_FRAME_HEIGHT,),
                'width'      : (IniFile.get_int, cv.cv.CV_CAP_PROP_FRAME_WIDTH,),
                'fps'        : (IniFile.get_int, cv.cv.CV_CAP_PROP_FPS,),
                'codec'      : (IniFile.get_str, cv.cv.CV_CAP_PROP_FOURCC,),
                'brightness' : (IniFile.get_int, cv.cv.CV_CAP_PROP_BRIGHTNESS,),
                'contrast'   : (IniFile.get_int, cv.cv.CV_CAP_PROP_CONTRAST,),
                'saturation' : (IniFile.get_int, cv.cv.CV_CAP_PROP_SATURATION,),
                'hue'        : (IniFile.get_int, cv.cv.CV_CAP_PROP_HUE,),
                'gain'       : (IniFile.get_int, cv.cv.CV_CAP_PROP_GAIN,),
                'exposure'   : (IniFile.get_int, cv.cv.CV_CAP_PROP_EXPOSURE,),
            }
...
#        del self.camera
    ...
    #-----------------------------------------------------------------------
    def set(self):
        width = IniFile.get_int("Camera", self.prefix+"_width",  0)
        if width: self.camera.set(3, width)
        height = IniFile.get_int("Camera", self.prefix+"_height",  0)
        if height: self.camera.set(4, height)
        self.angle = IniFile.get_int("Camera", self.prefix+"_angle")//90 % 4
        self.rotation = IniFile.get_float("Camera", self.prefix+"_rotation")
        self.xcenter = IniFile.get_float("Camera", self.prefix+"_xcenter")
        self.ycenter = IniFile.get_float("Camera", self.prefix+"_ycenter")
